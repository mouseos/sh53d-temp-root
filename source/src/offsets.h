...
struct kernel_offsets {
  const char *uname_r;
  /* Which compile-time layout (TARGET_CONFIG_H) this entry's kernel needs.
   * Only main.c re-points the *_OFF macros at this table; the struct field
   * offsets and physmap constants come from the target header, so matching
   * uname_r is not enough -- select_offsets() also requires this to equal the
   * built-in TARGET_LAYOUT_ID. */
  const char *layout;
