...
static int slide_commit_virtual_base(uint64_t base, const char *source) {
  if ((base >> 48) != 0xffff || base < KIMAGE_TEXT_BASE ||
      ((base - KIMAGE_TEXT_BASE) & (SLIDE_ALIGNMENT - 1)) != 0 ||
...
static int slide_commit_stext(uint64_t stext, const char *source) {
  ...
  if (slide > SLIDE_MAX_OFFSET ||
      (slide & (SLIDE_ALIGNMENT - 1)) != 0) {
...
#if defined(APP_REQUIRE_FRESH_P0_SESSION) && APP_REQUIRE_FRESH_P0_SESSION && \
    defined(APP_TRUST_FORCED_P0_SESSION) && APP_TRUST_FORCED_P0_SESSION
    /* The caller obtained this offset from the stock tracefs leak in the
     * same boot. Permit diagnostic use without physical fingerprinting. */
    slide_p0_session_fresh = 1;
#endif
#if defined(APP_PHYS_VIRTUAL_BASE_ORACLE) && APP_PHYS_VIRTUAL_BASE_ORACLE
...
#if defined(APP_REQUIRE_FRESH_P0_SESSION) && APP_REQUIRE_FRESH_P0_SESSION && \
    defined(APP_TRUST_FORCED_P0_SESSION) && APP_TRUST_FORCED_P0_SESSION
    /* The caller supplied the tracefs-derived virtual slide for this boot. */
    slide_p0_session_fresh = 1;
#endif
