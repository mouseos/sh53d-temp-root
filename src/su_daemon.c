...
#define BOOTSTRAP_SOCK_NAME "cve43499_su"
...
static socklen_t bootstrap_socket_address(struct sockaddr_un *sun) {
  memset(sun, 0, sizeof(*sun));
  sun->sun_family = AF_UNIX;
#if defined(ROOT_UMH_ABSTRACT_SOCKET) && ROOT_UMH_ABSTRACT_SOCKET
  memcpy(sun->sun_path + 1, BOOTSTRAP_SOCK_NAME,
         sizeof(BOOTSTRAP_SOCK_NAME) - 1);
  return (socklen_t)(offsetof(struct sockaddr_un, sun_path) +
                     sizeof(BOOTSTRAP_SOCK_NAME));
#else
...
static int connect_daemon(void) {
  ...
  socklen_t sun_len = bootstrap_socket_address(&sun);
...
static int set_selinux_permissive(void) {
  ...
  char value = '0';
  int ok = write(fd, &value, sizeof(value)) == (ssize_t)sizeof(value);
  if (ok && lseek(fd, 0, SEEK_SET) >= 0) {
    value = '1';
    ok = read(fd, &value, sizeof(value)) == (ssize_t)sizeof(value) &&
         value == '0';
  } else {
    ok = 0;
  }
...
static void seed_security_avc(void) {
